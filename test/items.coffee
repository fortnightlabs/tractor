...
module.exports =
  ...
  'initial updateTotals': (beforeExit, assert) ->
    items = newItems()
    assert.eql 2, items.totals.count
    assert.eql 90, items.totals.unassigned

  'add updateTotals': (beforeExit, assert) ->
    items = new Tractor.Items

    assert.eql 0, items.totals.count

    items.add new Tractor.Item(duration: 30, app: 'WebKit')
    assert.eql 1, items.totals.count
    assert.eql 30, items.totals.unassigned
